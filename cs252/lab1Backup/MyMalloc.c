...
void * allocateObject(size_t size)
{
    ...
    if (found == 1){
	    ...
	    if (temp->_objectSize > (roundedSize + 8 + sizeof(ObjectHeader))){
	    	    //make new block the allocated block
		    //setting 
	    ...
	    pthread_mutex_unlock(&mutex);
	    return (void *)((char *)temp + sizeof(ObjectHeader));
    } else { //if at the end allocate more memory 
	    //GetTopHeader of old space
	    char* _oldTopMem = (char*) _memStart - sizeof(ObjectHeader);
	    ObjectHeader * oldTopHead = (ObjectHeader *) _oldTopMem;

	    //Start and request new memory block
	    void *_mem = getMemoryFromOS(arenaSize);
	    atexit(atExitHandlerInC);
	    ...
	    fencePostFoot->_allocated = 1;
	    fencePostFoot->_objectSize = 0;
	    fencePostFoot->_listNext = oldTopHead; //Connect new footer to old header
	    oldTopHead->_leftObjectSize = sizeof(ObjectHeader);
	    ...
	    //TODO:Set the start of the current allocated block
	    printf("DummyStatement\n");
...
{
	//Set up pointers and local trackers
	...
	//find the block with largest address less than ptr
	ObjectHeader * _freeHolder = _freeList->_listNext;
	while(_freeHolder->_listNext < _ptrHead && _freeHolder->_listNext != _freeList){
		_freeHolder = _freeHolder->_listNext;
		printf("ohShit");
	} //At the end of this _freeHolder should point to the closest free block behind ptr
	
	//Declare the current block as unallocated
	_ptrHead->_allocated = 0;
	
	//Insert to list	
	_ptrHead->_listNext = _freeHolder->_listNext;
	_ptrHead->_listPrev = _freeHolder;
	_ptrHead->_listPrev->_listNext = _ptrHead;
	_ptrHead->_listNext->_listPrev = _ptrHead;
	
	//Check and react left allocation
	if(un_left == 0){
		//_freeHolder = _freeHolder->_listPrev; //compensating for _freeHolder misplacement
		size_t newSize = leftHeader->_objectSize + _ptrHead->_objectSize;
		leftHeader->_objectSize = newSize;
		_ptrHead->_objectSize = newSize;
		leftHeader->_listNext = _ptrHead->_listNext;
		_ptrHead->_listNext->_listPrev = leftHeader;
	}

	//Check and react right allocation
	if(un_right == 0){
		size_t newSize = rightHeader->_objectSize + rightHeader->_listPrev->_objectSize;
		rightHeader->_listPrev->_objectSize = newSize;
